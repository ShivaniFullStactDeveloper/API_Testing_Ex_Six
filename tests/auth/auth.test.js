...
const { TEST_USERS } = require('../../tests/utils/constants');
const {
    ensureAuthTestContext,
    getPreContextToken,
    getAccessToken
} = require('../../tests/helpers/token.helper');
...
describe('AUTH API TESTING', () => {                                                                                
    ...
    test('AUTH_16 - Select context with invalid tenant_id', async () => {
        ...
        const context = await ensureAuthTestContext();
        const token = await getPreContextToken();
